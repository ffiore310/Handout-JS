...
async function main() {
    ...
    if (token) {
      ...
      //Prefixo comum
        ...
        //Caca tesouro
        link = exercises['caca-ao-tesouro'].entrada.inicio;
        let result = await caca_tesouro(link, token);
        const r_caca = await postResposta('caca-ao-tesouro',result, token);
        console.log('Resultado caca ao tesouro:', r_caca);
      }
  }
...
async function caca_tesouro(link, token){
    const config = {
        headers: {
            'Content-Type': 'application/json',
            'Accept': 'application/json',
    ...
    entrada = link;
    while (cacando){
        result = await axios.get(entrada,config);
        // console.log(result);
